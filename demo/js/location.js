OC.location = {
    ...
    init: function(){
        $("#form_location_non_home_occ_check_site").submit(function(e){
            e.preventDefault();
            var address = $(this).find("input[name=business_physical_address_street]").val();
            address += " "+$(this).find("input[name=business_physical_address_detail]").val();
            OC.location.getZoneFromAddress(address)
        });
    },
    getZoneFromAddress: function(address){
        ...
        $.ajax(url, {jsonp:"callback", dataType:"jsonp", success:OC.location.setRegion}, "jsonp");
    },
    ...
    in_sola:function(pt){
	    return OC.location.ptInPoly(pt, OC.location.sola);
    },
    in_downtown: function(pt){
	    return OC.location.ptInPoly(pt, OC.location.downtown);
    },
    ptInPoly: function(pt, polyCords){
        var pointX = pt[0];
        var pointY = pt[1];
	    var i, j, c = 0;
